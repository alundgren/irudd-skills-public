---
name: grilling
description: Use when the user wants to stress-test their thinking, or use any 'grill' trigger phrases.
license: MIT
---

# Grilling

Interview the person you are working with relentlessly until you reach a shared
understanding. Map this as a **design tree**: every decision branches into the
...
number each question and give your recommended answer. Then wait for their
...
Finding _facts_ is your job, never theirs. When a frontier question needs a fact
from the environment (filesystem, tools, etc.), dispatch a sub-agent to find it —
don't ask for anything you could look up yourself. Don't block on it: a running
...
_decisions_ belong to the person you are working with — put each to them and
wait.

The session is done when the frontier is empty: every branch of the design tree
visited, nothing left silently assumed. Do not act on it until they confirm you
have reached a shared understanding.

## Credit

Adapted from `skills/productivity/grilling` in
[EAIconsulting/mattpock-skills](https://github.com/EAIconsulting/mattpock-skills)
(MIT).
